...
import { newsSummarize, newsSummarizeLive } from '../api'
...
export default function NewsSummarizer({ symbol }) {
  ...
  // Parse pasted lines into {title, snippet}
  const items = useMemo(() => {
    return text
      .split('\n')
      .map(l => l.trim())
  ...
  // Summarize user-pasted headlines
  const summarizePasted = async () => {
    if (!items.length) return
    setLoading(true); setErr(null); setOut(null)
    try {
      const data = await newsSummarize({ symbol, items })
      setOut(data)
    } catch (e) {
      setErr(e.message || 'News summarize failed')
    } finally {
      setLoading(false)
    }
  }

  // Fetch & summarize live headlines (server pulls feeds)
  const fetchLatest = async () => {
    if (!symbol) return
    setLoading(true); setErr(null); setOut(null)
    try {
      const region = /\.NS$/i.test(symbol) ? 'IN' : 'US'
      const data = await newsSummarizeLive({ symbol, n: 10, region, lang: 'en' })
      setOut(data)
    } catch (e) {
      setErr(e.message || 'Live news fetch failed')
    } finally {
      setLoading(false)
    }
  }
  ...
  const badgeClass =
    out?.sentiment === 'positive' ? 'bullish' :
  ...
  return (
    <div className="card" style={{ display:'flex', flexDirection:'column', gap:10 }}>
      ...
      {/* Optional paste area (manual mode) */}
      <textarea
        value={text}
